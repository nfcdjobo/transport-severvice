window.addEventListener(`DOMContentLoaded`, (e)=>{
    ...
    if(!localStorage.SESSION_TRANSPORT){
        ...
        if(location.href.includes('machinistes.html')){
            function editeLigne(event){location.href=`./editeMachinistes.html#${event.target.id}`}
            function deleteLigne(event){
                if(confirm(`Êtes-vous sûre de vouloir supprimer cet utilisateur ?`)){
                    $.ajax({
                        type:`POST`,
                        url:`${urlApi}deleteMachiniste`,
        ...
        function afficher(donnees){
            ...
            donnees.forEach((element, key) => {
                ...
                `<tr data-index="${key+1}" id="tr-${element._id}">
                    ...
                    <td class="datatable-ct"><a href="javascript:void(0)" data-name="role" data-pk="undefined" data-value="machiniste" class="editable editable-click">machinisteistrateur</a></td>
        ...
        if(location.href.includes(`addMachinistes.html`)){
            ...
            $(`#retour`).click((event=>{
                event.preventDefault();
                if(location.href.includes(`editeMachinistes`) || location.href.includes(`addMachinistes`)) location.href=`./machinistes.html`;
            }))
        ...
        if(location.href.includes(`editeMachinistes.html`)){
            $(`#retour`).click((event=>{
                event.preventDefault();
                if(location.href.includes(`editeMachinistes`) || location.href.includes(`addMachinistes`)) location.href=`./machinistes.html`;
            }));
            ...
            $.ajax({
                type:`GET`,
                url:`${urlApi}getByIdMachiniste/${location.href.split('#')[1]}`,
                headers: {Authorization: `token ${JSON.parse(localStorage.SESSION_TRANSPORT).body.token}`}
            })
            .then(response=>{
                const machinistes=response.data;
                document.getElementById('nomPrenom').value=machinistes.nomPrenom;
                document.getElementById('email').value=machinistes.email;
                document.getElementById('telephone').value=machinistes.telephone;
                document.getElementById('photoResponsive').value=machinistes.photo;
                document.getElementById('role').value=machinistes.role;
                document.getElementById('id').value=machinistes._id;
                document.getElementById('photoResponsive').src=imgPath+machinistes.photo;
            })
            ...
            $(`#formulaireAdd`).submit(async event=>{
                ...
                let all= await fetch(urlApi+'updatemachiniste', {
                    method: 'POST',
                    body: formData,
                    headers: {Authorization: `token ${JSON.parse(localStorage.SESSION_TRANSPORT).body.token}`}
                })
